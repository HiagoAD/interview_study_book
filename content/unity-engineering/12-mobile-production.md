...
Check whether material access creates new material instances. Reading `Renderer.material` copies the material for that renderer, and destroying the copy becomes your code's job; `Renderer.sharedMaterial` returns the shared asset without copying. Accidental copies can use more memory and prevent intended sharing. Changing a shared material has the opposite risk: it affects every renderer that uses it. Per-renderer values set through a `MaterialPropertyBlock` avoid the copy, but they take that renderer out of the SRP Batcher. Choose who owns the material or per-instance data, then verify the result's batching behavior in the project's pipeline.
...
Two consequences follow, and both are practical. Opaque geometry is cheap to overlap, because the hardware can often discard hidden fragments before shading them. Transparency is not. Blending reads what is already in the tile, which is on chip and cheap, but every transparent layer still has to be shaded, because hidden-surface removal cannot discard a fragment that blends with what lies behind it. A stack of large transparent particles is therefore expensive in a way that the same area of opaque geometry is not, and this is why the advice in this section keeps returning to overdraw from transparent effects rather than to polygon counts. A match-3 board is the quiet version of the same bill. Tile quads, each carrying its own glow, highlight, and selection overlay, can stack six or seven blended layers across most of the screen with no particle system in the scene at all, and that cost is paid on every frame the board is visible rather than only while an effect plays.
...
GPU compression changes that estimate, and supported formats differ by target. Read/write settings can keep an additional CPU-accessible copy for supported resources. Mipmaps use more memory but improve sampling when a texture is displayed smaller. A UI texture that is never reduced in size may therefore need different settings from a world texture seen at many distances. The genre usually decides which setting dominates. A 2D puzzle spends most of its texture budget on board and interface art drawn at a fixed size, so its wins come from atlasing and from omitting mip chains nothing samples. A 3D runner spends its budget on world textures seen at many distances, where the mip chain earns its extra third and the compression format is the decision that matters. Carrying one genre's import settings into the other reliably buys memory that no draw call reads. [Unity's texture compression reference](https://docs.unity3d.com/6000.0/Documentation/Manual/texture-compression-formats.html) explains platform-specific choices.
...
Frame pacing describes when completed frames reach the display. Stable 30 FPS can feel more consistent than a higher rate that repeatedly misses its deadlines. Choose a target that fits the display's refresh behavior and the workload the device can sustain. In Unity that target is `Application.targetFrameRate`. Android and iOS ignore `QualitySettings.vSyncCount`, so the target is the only control there, and at its default of -1 they render at a fixed 30 FPS to conserve battery, whatever the display's refresh rate. On desktop the rule reverses: a nonzero `vSyncCount` makes Unity ignore the target. Android's [Unity development guide](https://developer.android.com/games/engines/unity/start-in-unity) discusses frame pacing and thermal behavior.
...
Be precise about which lifecycle callbacks are guaranteed, because the design depends on it. A focus or pause callback, `OnApplicationFocus(false)` or `OnApplicationPause(true)`, usually arrives when the player leaves the app, and that is a good moment to save. A quit callback is not reliably delivered on mobile: `OnApplicationQuit` is not called on iOS, where apps are suspended rather than quit, and the operating system can reclaim a backgrounded process without running any further application code. Design so that losing everything after the last pause callback is acceptable, and treat anything the quit callback does as a convenience.
