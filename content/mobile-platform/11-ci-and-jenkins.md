...
`archiveArtifacts` keeps files on the controller with the build, for as long as the build's log is kept, and fails the build when it finds nothing to archive, unless `allowEmptyArchive` is set. So `success` archives the artifacts that must exist, and `always` archives the logs, which a failed build may not have written. Test results need a step that reads them. Jenkins's `junit` step reads JUnit's XML format, and Unity's results are NUnit's, so the pipeline uses the NUnit plugin's `nunit` step. By default it fails the build when it finds no results file, or files that hold no result: zero tests is not a pass, in Jenkins's own terms.
...
* The encoded token in the clear, since masking matches the raw text
...
* Not reliably: masking knows the text and a few shells' quoted forms
...
- An agent may have no graphics device. Under `-nographics` there is none, and a test or build step that needs one fails.
...
- The merged manifest, which lists the classes under their full names
