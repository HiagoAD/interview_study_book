...
| Unity build | Runs Unity in batch mode, with a build script that builds the variant's build profile with the version and signing that the pipeline supplies | The exit code, the build report's result, and the output at its path |
...
`-batchmode` runs the Editor with no need for a person and suppresses the dialogs that would wait for one, `-nographics` runs it without initializing a graphics device, which an agent may not have, `-projectPath` names the project, and `-logFile` writes the Editor's log to a file that the pipeline keeps. A build adds three more: `-quit` closes the Editor once the other arguments have run, `-buildTarget` selects the platform that the Editor opens the project for, and `-executeMethod` calls a static method of an Editor script. Unity's [command-line reference](https://docs.unity3d.com/6000.3/Documentation/Manual/EditorCommandLineArguments.html) lists the others. The script assumes macOS agents with the Hub's default install folder; on another operating system only the path changes.

```bash
ci/unity.sh build-android -quit -buildTarget Android -executeMethod Game.Pipeline.CiBuild.Android
```

The method builds one of chapter 10's variants, which are build profiles ([[#release-build-variants]]), with values that the pipeline passes in environment variables:
...
namespace Game.Pipeline
{
    public static partial class CiBuild
    {
        public static void Android() => Run(() =>
        {
            ...
            BuildActiveProfile("build/Game.aab");
        });
        ...
        public static void IOS() => Run(() =>
        {
            ActivateProfile();
            ApplyVersion();
            BuildActiveProfile("build/xcode");
        });

        // The variant is a build profile. Player settings calls act on the active profile,
        // so the profile to build becomes active before anything is set.
        static void ActivateProfile()
        {
            string path = Env("BUILD_PROFILE");
            BuildProfile profile = AssetDatabase.LoadAssetAtPath<BuildProfile>(path);
            if (profile == null)
            {
                throw new InvalidOperationException("No build profile at " + path);
            }

            BuildProfile.SetActiveBuildProfile(profile);
        }

        // The pipeline decides the version and the build number.
        static void ApplyVersion()
        {
            PlayerSettings.bundleVersion = Env("GAME_VERSION");
            int buildNumber = int.Parse(Env("GAME_BUILD_NUMBER"), CultureInfo.InvariantCulture);
            PlayerSettings.Android.bundleVersionCode = buildNumber;
            PlayerSettings.iOS.buildNumber = buildNumber.ToString(CultureInfo.InvariantCulture);
        }

        static void BuildActiveProfile(string path)
        {
...
Every value the build depends on comes from the pipeline, and a missing one stops the build rather than letting it fall back on whatever the project's settings hold. `BUILD_PROFILE` names the variant, and the profile decides the rest: its build settings make it a development build or not, and its [[scripting define symbol|scripting defines]] choose the environment ([[#release-environments]]), which chapter 10's check verifies as the build starts. The script keeps the order of chapter 10's `StoreBuild` ([[#release-versioning]]): the profile becomes active before anything is set, since Player settings calls act on the active profile, and the numbers are written after that. The build stamp that `StoreBuild` also writes belongs at the same point; it is left out here, to keep the script to what a pipeline adds: the signing, and a failing exit code for every failure.

`BuildPipeline.BuildPlayer` fails in three ways, which Unity's reference source for 6000.3 shows; the overload that takes a profile makes the profile active and then builds through the same path. It reports most failures in the result of the `BuildReport` it returns. It returns null after logging some errors that arise during the build. And it throws for others before it starts, such as an output path that the target does not accept, or another build already in progress. The script treats all three as failures and ends the Editor with `EditorApplication.Exit(1)`, as Unity's own code does when an exception it does not expect reaches it. It tests for `Succeeded` rather than for `Failed`: in a Unity 6.3 test, a build that stopped at its start, with no scene to build, returned a report whose result was `Unknown`. `BuildOptions.StrictMode` fails the build on any error, and it adds to the options that the profile sets.
...
* So that a failed build fails the stage, whatever Unity does next
...
- A copy of each agent's Library folder, to compare the imports
...
pipeline {
    ...
    parameters {
        choice(name: 'VARIANT', choices: ['QA', 'Store', 'Development'], description: 'The build profile to build; a Store build also goes to the test tracks')
    }
        ...
        stage('Build') {
            parallel {
                stage('Android') {
                    ...
                    environment {
                        BUILD_PROFILE = "Assets/Settings/Build Profiles/Android ${params.VARIANT}.asset"
                    }
                    stages {
                        stage('Android clean checkout') {
                            when { expression { params.VARIANT == 'Store' } }
                            steps {
                                deleteDir()
                                checkout scm
                            }
                        ...
                        stage('Android upload') {
                            when { expression { params.VARIANT == 'Store' } }
                            steps {
                                sh 'ls build/Game_mapping.txt build/*.symbols.zip'
                                sh 'ci/check-store-hosts.sh build/Game.aab'
                    ...
                    post {
                        ...
                        success {
                            archiveArtifacts artifacts: 'stamp.env, build/Game.aab, build/Game_mapping.txt, build/*.symbols.zip', fingerprint: true
                        }
                    }
                }
                stage('iOS') {
                    ...
                    environment {
                        BUILD_PROFILE = "Assets/Settings/Build Profiles/iOS ${params.VARIANT}.asset"
                    }
                    stages {
                        stage('iOS clean checkout') {
                            when { expression { params.VARIANT == 'Store' } }
                            steps {
                                deleteDir()
                                checkout scm
                        ...
                        stage('iOS upload') {
                            when { expression { params.VARIANT == 'Store' } }
                            steps {
                                sh 'ls -d build/Game.xcarchive/dSYMs/*.dSYM'
                                sh 'ci/check-store-hosts.sh build/ipa/*.ipa'
...
Each stage takes an agent by label: the tests any agent labeled `unity`, Android one labeled both `unity` and `android`, and iOS a Mac. The two branches of `parallel` run at once, each on its own agent with its own workspace, and each runs its stages in order, a clean checkout, the build and the upload, where `when` skips the first and the last unless the variant is Store. A stage inside `parallel` can have its own agent, `when` and nested stages, but not a `parallel` of its own. A stage with its own agent evaluates `when` after it takes the agent, unless `when` holds `beforeAgent true`.

The `VARIANT` parameter names one of chapter 10's build profiles, and each branch's `environment` turns it into the path of that platform's profile, which the build script reads. One parameter chooses the environment and the build type together, because the profile fixes both ([[#release-build-variants]]); two separate parameters could combine a release build with staging, the combination that chapter 10's check exists to refuse. Each build stage deletes `build` before it builds, for the reason that the test stage deletes `results`: the `test -s` that follows must not find an older run's bundle. A Store build goes further and starts from an empty workspace and a fresh checkout, since release candidates build clean, as the section on caching explains.

`stash` saves files on the controller for any agent of the same run, and `unstash` brings them into the current workspace. The Prepare stage writes the build's stamp once, and each branch unstashes it, so both platforms get the same version and build number. A stash is discarded when the run ends, and it is meant for small files: Jenkins's help for the step, as read in September 2026, sets no hard limit, advises alternatives from somewhere between 5 and 100 MB, and explains that the controller compresses each stash, which costs it CPU time. That decides how a project moves between agents. The Xcode project that Unity exports, which [[#xcode-build-flow]] archives, holds the game's data and [[IL2CPP]]'s C++, so this Jenkinsfile keeps the export and the archive on one Mac, and a pipeline that splits them between agents passes the folder through an artifact repository, as the same help suggests for large transfers.

`archiveArtifacts` keeps files on the controller with the build, normally for as long as the build's log is kept, though `buildDiscarder` can drop artifacts sooner, as this Jenkinsfile's `artifactNumToKeepStr` does. It fails the build when it finds nothing at all to archive, unless `allowEmptyArchive` is set, and a pattern in its list that matches nothing is skipped: in a local Jenkins 2.568.3, a list whose mapping file and symbols zip were missing archived the rest and passed. So `success` archives the artifacts, while each file that must exist has a check of its own, the `test -s` after the build and the `ls` of the symbol files before an upload, and `always` archives the logs, which a failed build may not have written. Test results need a step that reads them. Jenkins's `junit` step reads JUnit's XML format, and Unity's results are NUnit's, so the pipeline uses the NUnit plugin's `nunit` step. By default it fails the build when it finds no results file, or files that hold no result: zero tests is not a pass, in Jenkins's own terms. For a file that holds no test, the step throws and ends its `post` block, so the stage archives its logs before it calls the step.
...
- The controller's built-in node, which `&&` selects for iOS builds
...
* Builds there can reach the controller's files as Jenkins itself can
- Builds there run slower, because the controller limits their CPU time
- Builds there get no credentials, which Jenkins binds on agents alone
- Builds there share one workspace, so parallel stages overwrite files
- Builds there skip their post blocks, since agents are what run them
...
* Each stage takes an agent with its tools, for as long as it runs
...
- A Jenkinsfile holds one label, so the other stages need their own
...
- None, since the agents of a run share the first stage's workspace
...
* Stashes are meant for small files, and each one costs the controller
- A stash holds single files, and an exported Xcode project is a folder
- `unstash` works on the agent that made the stash and on no other
...
* Failed builds keep no logs, and those are the logs someone needs
...
- Yes: Jenkins records a stopped run with the failed status
...
The keystore holds the upload key. With [[Play App Signing]], Google signs what players install with a key of its own, so a leaked upload key can be reset, but a leak is still a leak, and the file stays in the credentials store and in the temporary directory that the binding deletes. Unity keeps the keystore's path in the project but not its passwords, so a build in a new Editor session fails at signing unless something sets them again, and the script sets all four on each run.
...
security set-key-partition-list -S apple: -s -k "$password" "$keychain"
...
?? ci-secret-masking A step inside `withCredentials` runs `printf %s "$TOKEN" | xxd -p`, which prints the token in hexadecimal. What does the build log show?
* The token in hexadecimal, unmasked, since masking knows fixed forms
- Asterisks, since Jenkins masks each encoding of a bound secret it holds
- Nothing, since `withCredentials` blocks commands that print secrets
- An error, since a bound variable is refused as input to another tool
- Asterisks, because the output of `xxd` is masked for bound credentials
> Masking replaces the secret's text, and a fixed set of its forms, such as the quoted forms that common shells print, with asterisks. Hexadecimal is not among them, so the token reaches the log in the clear, and hexadecimal is trivially decoded. Masking guards against accidents; the pipeline itself never prints a secret.

?+ Why do Jenkins's examples pass a secret to `sh` in single quotes rather than double quotes?
* Single quotes leave the variable to the shell, not Groovy
- Double quotes switch off masking for the rest of the step's output
- Single quotes make the shell encrypt the variable before it is used
- Groovy refuses double-quoted strings inside a `withCredentials` block
- Single quotes keep the secret out of the step's environment variables
...
* Not reliably: masking knows the text and a fixed set of its forms
...
- As protection from anyone who is able to edit the Jenkinsfile
- As a reason to allow debug logging of the full environment in CI
- As encryption of the bound secret files on the agent's disk
- As a way to share secrets safely through the archived artifacts
...
* The key lasts one build, in a keychain whose password the stage holds
- `codesign` trusts identities in keychains made during the build
...
* The keychain holding the key, since the later steps did not run
...
- The certificate alone, since the private key stays in the `.p12` file
...
* Without `-P`, it asks for the passphrase in a window that nobody sees
...
- A `.p12` file has to be imported through Xcode's account settings
...
The main cache is Unity's `Library` folder, which holds the imported form of the project's assets and the other data that Unity derives from the project. Unity's manual says that it behaves like a cache: it stays out of version control, and it can be deleted while the project is closed, since Unity regenerates it from `Assets` and `ProjectSettings`, at the cost of a full import. The build's intermediate files are there too, under `Library/Bee`, the C++ that IL2CPP generates and the native code compiled from it among them. Jenkins leaves a workspace in place after a build, so a job that runs on the same agent finds its `Library` from the last run. A team that builds on many agents, or on fresh ones, restores a saved `Library` at the start of a build, and the Unity Accelerator, a caching proxy that keeps copies of a team's imported assets on the local network, lets an agent download what another machine has imported instead of importing it again. With `-quit`, Unity's command-line reference recommends `-cacheServerWaitForUploadCompletion`, so that the Editor stays open until its uploads to the Accelerator finish.
...
The native tools keep caches on the agent too. Gradle keeps the dependencies it downloads in caches under its user home, and a daemon, a process that stays running between builds and keeps state from one to the next; two builds that share them share that state, which the next section returns to. [[CocoaPods]] caches downloads in `~/Library/Caches/CocoaPods`, or where the `CP_CACHE_DIR` variable points. All of them grow, and so do the workspaces and their `Library` folders, and an agent with a full disk fails in whichever step writes next. So a pipeline checks the free space at the start of each build and prunes old caches on a schedule.
...
- The branch name, since each branch needs a Library of its own
...
> What Unity derives into `Library` depends on the Editor version, the target's import settings and the packages, so a key that names all three restores a `Library` made under the same conditions. A key by commit matches only a rebuild of the same commit, never the next commit's build, and a key by date, branch or agent restores a `Library` made under other conditions.
...
- The new release refuses any cache, so caching has to stop for good
...
- Retry with the same cache until a build passes, then keep it
...
* So that the commit and the tools alone decide what ships
- Because the stores reject builds that were made from cached imports
- Because a build from a cache is refused for distribution signing
...
- Reinstall Unity on the agents, since their installation may be damaged
...
- The agent's file system compressed the texture during the checkout
...
* The workspace kept the results file of an earlier build
...
* Their file systems ignore the case of names, and the agent's does not
...
- The Macs restored a cached Library that held the texture
...
- Nowhere yet: the stage has to run again with more logging
...
Chapter 10 set the rules for the two version values ([[#release-versioning]]): the marketing version names a release, and the build number tells uploads apart, rises with each upload of the app and never repeats, so CI assigns it. This pipeline keeps the marketing version in a `VERSION` file in the repository, the one source of truth, which the Prepare stage reads without starting Unity and the build script writes into Player settings. For the build number it takes Jenkins's `BUILD_NUMBER`, which fits when one job makes every upload, as chapter 10 explains, and adds a base number kept in the repository. A recreated job counts from 1 again, so before its first run the base is raised, in a commit, to the last uploaded number, and the numbers keep rising. The Prepare stage writes the stamp that every later stage reads:
...
For a release candidate, the pipeline tags the commit with the version and build number, such as `v1.8.0-512`, archives the artifacts and the symbols, and uploads the candidate to the test tracks. Before each upload, the Jenkinsfile takes the second look at the finished build that chapter 10 left to this chapter ([[#release-environments]]), a search of the store build's files for the hosts of development and staging:

```bash
#!/bin/bash
# ci/check-store-hosts.sh <AAB or IPA>: fails when a store build holds the host of
# development or staging as a plain string, in any file inside it.
set -euo pipefail
files="$(mktemp -d)"
trap 'rm -rf "$files"' EXIT
unzip -q "$1" -d "$files"
status=0
grep -r -a -F -l -e api.dev.example.com -e api.staging.example.com "$files" || status=$?
case "$status" in
  0) echo "$1 holds a development or staging host, in the files listed above" >&2; exit 1 ;;
  1) ;;   # neither host is in the build
  *) echo "the search of $1 failed" >&2; exit 1 ;;
esac
```

The script unzips the bundle or the IPA, both of which are zip files, and fails when any file inside holds one of the hosts. `grep` exits with 0 for a match, 1 for none and more than 1 for an error, and the script fails on an error too, so a search that did not run cannot pass. In a Unity 6.3 test, it passed a store bundle and failed a QA bundle, whose staging address sat in the `global-metadata.dat` that [[IL2CPP]] writes. It finds only what sits in the build as a plain string, which is why chapter 10's check of the profile's defines comes first, as the build starts.

Google Play's Developer API works through an edit: a program creates one, uploads the bundle to it, puts the bundle's version code in a release on a track such as internal testing, and commits the edit. [[App Store Connect]] receives the IPA for [[TestFlight]] through one of the upload paths that [[#xcode-build-flow]] lists, with an API key from the credentials store. Promotion to production stays a person's decision, made in the stores' consoles or in a pipeline stage whose `input` waits for someone to approve it, after the candidate has been checked on the test tracks as [[#release-verification]] describes.

Build the candidate once and promote that artifact. On Google Play, a release on a track lists the version codes of bundles already uploaded, so promoting a candidate to production means releasing the same version code there, with no new upload. On the App Store, a version is submitted for review with one of the builds already uploaded ([Apple's guide](https://developer.apple.com/help/app-store-connect/manage-builds/choose-a-build-to-submit)), the one testers had in TestFlight. Rebuilding the commit for release makes a new binary that nobody tested: a tool, a cache or a resolved dependency can differ between two runs, and the store needs a new build number for the new upload anyway.
...
* It makes a new binary that nobody tested, from inputs that can drift
...
- A rebuild gives a byte-identical file, so it merely wastes build time
...
* Uploaded 498's symbols at build time, and kept a copy outside Jenkins
- Nothing: symbols can be rebuilt later from the commit tagged for 498
- Kept the IPA, from which the dSYMs can be extracted when they are needed
- Asked the stores for the build's symbols, which they keep for many years
- Nothing: the stores name the frames of a crash without the build's symbols
...
?+ Months after a release, the game's crash reporter shows obfuscated Java frames from it, and nobody uploaded that build's mapping file to it. Where does the team find the file?
* In the pipeline's own archive of that build: its bundle or its mapping file
- In the latest build's mapping file, since R8 renames the same classes
- In Google Play, which passes mapping files on to other crash reporters
- In the native symbols zip, which maps Java names as well as native ones
- In the project's [[Gradle]] templates, which record the names that R8 chose
> R8 writes a mapping file for each build that it minifies ([[#gradle-r8-symbols]]), and another build's file retraces into wrong names without a warning. The bundle carries its own build's mapping file, and the pipeline keeps the file with that build's other symbols outside Jenkins, by version and build number, so it can still be uploaded to the crash reporter.
