...
| Attempt | One request and its response | The game's own timer, which cancels the attempt. `UnityWebRequest.timeout` stays a guard against a stalled request |
...
The attempt and the operation are the pair that matters most. `UnityWebRequest.timeout`, a whole number of seconds set before the request is sent, cannot be the attempt's limit on every platform: chapter 8 found that the Editor applies it to the whole transfer, while iOS hands it to an idle timeout, which a response that keeps arriving a few bytes at a time never trips. So the game times each attempt itself and leaves the property to the transport, as its guard against a stall. A retry is a new request with a timer of its own, so a per-attempt timeout bounds one attempt and nothing more. Four attempts with a 10-second timeout, and waits of up to 1, 2 and 4 seconds between them, can hold the player for 47 seconds. In a .NET 8 test against a server that never answered in time, three attempts with a 1-second timeout took 3.0 seconds, while a deadline of 1.5 seconds across the same loop ended it after two attempts, at 1.5 seconds.
...
The owner of an operation can end it before any limit does: the player leaves the screen, and the object that asked for the data is destroyed. The `destroyCancellationToken` of [[#platform-results-threads]] is cancelled when its `MonoBehaviour` is destroyed. Chapter 8's transport aborts its request when the token it was given is cancelled, and its session layer passes the token down to it, so an attempt links the owner's token to a timer of its own and hands the linked token to the `SendAsync` of either layer, which have the same shape:

```csharp
// One attempt: its own timer, clipped to the operation's deadline, and cancelled with its owner.
static async Task<ApiResponse> SendAttemptAsync(Func<ApiRequest, CancellationToken, Task<ApiResponse>> send,
    ApiRequest request, TimeSpan attemptLimit, DateTime deadlineUtc, CancellationToken owner)
{
    TimeSpan left = deadlineUtc - DateTime.UtcNow;
    if (left <= TimeSpan.Zero)
        throw new TimeoutException("The operation's deadline has passed.");

    using var attempt = CancellationTokenSource.CreateLinkedTokenSource(owner);
    attempt.CancelAfter(left < attemptLimit ? left : attemptLimit);
    try
    {
        return await send(request, attempt.Token);
    }
    catch (OperationCanceledException) when (!owner.IsCancellationRequested)
    {
        // The owner is still waiting, so the timer fired: no response, which the policy may retry.
        return ApiResponse.NoResponse("attempt timed out");
    }
}
```

`CancelAfter` fires on a thread-pool thread, and `UnityWebRequest` belongs to the main thread. In a Unity 6.3 test in the Editor, the transport still resumed on the main thread after the timer aborted its request, and Unity reported no error. The attempt ended at its limit both when the server was slow to answer and when it sent its body ten bytes a second.
...
> Cancelling ends the client's waiting, not necessarily the server's work. A request that reached the server may run to its end there or may have committed already, as a .NET test showed when its server finished both requests whose client had stopped waiting. The record stays pending, with its key, until an answer settles it.
...
A retry helps when two things hold: the failure may pass, and a second copy of the request does no harm. The second depends on the method. Chapter 8 set out the rule from [RFC 9110](https://www.rfc-editor.org/rfc/rfc9110.html#name-idempotent-methods): PUT, DELETE and the safe methods, GET and HEAD among them, are [[idempotent]], so that sending one twice has the same intended effect as sending it once, and POST is not. A client does not retry a non-idempotent request unless it knows that the request is idempotent all the same, or that it was never applied. A POST that carries an idempotency key, the subject of the next section, counts as idempotent here:
...
| 500 Internal Server Error, 502 Bad Gateway, 503 Service Unavailable, 504 Gateway Timeout | Only an idempotent request, after the wait that `Retry-After` gives, if any | The status does not say whether the work was done: a server can fail partway through, and a gateway may have forwarded the request before it failed |
| 408 Request Timeout | Yes | The server did not receive the whole request, and HTTP lets the client repeat it |
| 429 Too Many Requests | Yes, after the wait that `Retry-After` gives, or the backoff | The server declined the request for now |
...
A 401 means that the request was not applied because its credentials were missing or no longer valid, usually an access token that has expired. Chapter 8's session layer handles it, below the policy: it refreshes the access token once, with one refresh shared by every request that needs it, sends the request once more, and passes a second 401 up as the answer, which sends the player to sign in again. The policy wraps the session layer, so each attempt goes out with the current token, and the policy itself never retries a 401. The one 409 that is retried is the one that a backend following the idempotency draft returns for a key whose first request is still running, which the next section covers. The client recognizes it by the error that the backend documents for it, since the same endpoint can answer 409 for an ordinary conflict with the resource's state, and that one repeats.

`Retry-After` gives either a number of seconds or an HTTP date. The policy waits at least that long, and when the wait would end after the operation's deadline, it stops now, since an attempt made after the deadline serves nobody. For `UnityWebRequest`, the first two rows look alike: each ends with `ConnectionError`, which chapter 8's transport returns as a response with status 0, and only the `error` text separates them. In a Unity 6.3 test in the Editor, that text read “Cannot resolve destination host” for an unknown host and “Cannot connect to destination host” for a refused connection. A policy that cannot tell whether a failed request left the device treats it as the second row, an unknown outcome.
...
- One layer retries: the layer that knows the operation, its deadline and its key. For the game's own requests that is its HTTP layer, and the layers below it retry less, or not at all, where their settings allow. A backend SDK sits behind one of the game's adapters, as chapter 7 places every SDK; if the SDK retries on its own, the adapter adds no loop around it, or turns the SDK's retries off where a setting allows. The SDK's documentation says what it retries, and a proxy on the device's traffic, as in chapter 7's evaluation, shows it.
...
public static class RetryRules
{
    ...
    // keyInFlight marks the 409 that the backend documents for a key whose first request still runs.
    public static bool ShouldRetry(RetryPolicy policy, int status, bool sent, bool keyInFlight)
    {
        if (status == 0)
        ...
        if (status == 409)
            return keyInFlight;       // any other conflict repeats on a retry
        if (status == 500 || status == 502 || status == 503 || status == 504)
            return policy.Idempotent; // the work may have been done, in part or in full
...
> A 401 means that the request was not applied because its credentials were missing or no longer valid, usually an expired access token. One refresh and one resend settle that case, and a second 401 sends the player to sign in again. A 403 is different: the server understood the request and refuses it, and a refreshed token, which carries the same permissions, changes nothing.
...
public sealed class PendingOperation
{
    ...
    public string Path;      // such as "/rewards/claim", sent as a POST
...
public static class Operations
{
    ...
    public static PendingOperation Create(string path, string body) => new PendingOperation
    {
    ...
    public static ApiRequest ToRequest(PendingOperation operation) =>
        new ApiRequest("POST", operation.Path, operation.Body)
            .WithHeader("Idempotency-Key", "\"" + operation.Key + "\"");
}
```
...
?+ A retry of a claim gets 409 Conflict, with the error its backend documents, as the idempotency draft describes, for a key whose first request is still running. What does the client do?
...
> Under the draft, this 409 means that the first request with the key is still running. The client corrects nothing: it waits and retries with the same key and body, and gets the stored result once the first request finishes. A new key would start a second operation.
...
- Over HTTPS, the portal cannot answer in the server's name. An intercepted connection fails the [[TLS]] certificate check with a host name mismatch, so the request ends as a connection error, which the game treats as being offline. A portal that blocks HTTPS instead of intercepting it leaves the request to time out, with the same result.
- Over plain HTTP, the portal can answer with its own page, as a 200 with HTML. `UnityWebRequest` refuses plain HTTP in a default project, as chapter 8 describes, so this is the case of a test server's `http://` address in a build whose “Allow downloads over HTTP” setting lets it through, such as a development build under `DevelopmentOnly` ([[#xcode-build-settings]]), or of `HttpClient`, which the setting does not reach.

Either way, a response is checked before it is used: the status is one the endpoint returns, `Content-Type` is the JSON it sends, and the body parses into the response's data type with its required fields present. A body that fails any of those checks counts as a failed attempt, and it must not reach the save, a cache or the player's balance. The checks do not prove who sent the response, which only HTTPS with a validated certificate does; they reject what the endpoint would not send, such as a portal's page, whatever answered in the server's place.
...
| Progress and rewards | Queued as pending operations with [[idempotency]] keys, sent in order on reconnection | Each must take effect exactly once, however many times it is sent, and the player sees it as pending until the server confirms it |
...
Chapter 8's conditional write makes the conflict visible instead of silent. The device keeps the `ETag` of the save it last downloaded and uploads with `If-Match` set to it; if another device saved in between, the server answers 412 Precondition Failed instead of overwriting, and the client downloads the newer version and merges it or asks the player. An upload without `If-Match` replaces the other device's save, the lost update that the condition exists to prevent.
...
- Nothing, since the platform validates a route before reporting it
...
?? network-captive-portal A player on hotel Wi-Fi has not signed in to its captive portal, which intercepts connections to show its sign-in page. The game's requests go to its backend over HTTPS. What does the game see?
...
> A portal's page can arrive with a 200, quickly, on a network that reports a normal route. What it does not do is look like the endpoint's JSON: a body with the wrong content type, or one that does not parse into the expected type with its required fields, is a failed attempt.
