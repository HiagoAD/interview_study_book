...
Options: correct longest in 20% of 41 sets, shortest in 20%, medians 67 and 66, and the word list only in the name of the `always` condition. Two of 5 new links return 200. The 3 blocked links, Unity's command-line arguments, licensing, `Library` and the Test Framework 1.6 runner wait under “To check on the Mac” in [evidence/11-ci-and-jenkins.md](evidence/11-ci-and-jenkins.md), beside the outline's errors, such as `junit` for Unity's NUnit results.
