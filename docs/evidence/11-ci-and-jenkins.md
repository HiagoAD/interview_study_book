...
- **The links the container could not fetch**, for `npm run guard -- links`: `https://docs.unity3d.com/6000.3/Documentation/Manual/EditorCommandLineArguments.html`, `https://www.jenkins.io/doc/book/pipeline/syntax/` and `https://www.jenkins.io/doc/book/pipeline/jenkinsfile/`. The other two new links, Android’s versioning guide and Apple’s page on choosing a build, return 200.
- **`PlayerSettings.bundleVersion`**: that it becomes `versionName` and `CFBundleShortVersionString`, from its 6000.3 reference page. The chapter says only that the build script sets the marketing version.
- **`-nographics` and case sensitivity**: that a test needing a graphics device fails under `-nographics`, and whether the Mac’s startup volume is case-insensitive (`diskutil info /`). The chapter states both as differences between machines, and one question relies on a case-insensitive Mac, which its scenario gives.
